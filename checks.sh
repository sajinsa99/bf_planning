#!/usr/bin/env bash
# Code quality and security checks for bf_planning.
...
write_report() {
  ...
  ts=$(date '+%Y-%m-%d %H:%M:%S')
  {
    printf '# Checks Report — bf_planning — %s\n\n' "$ts"
    printf '## Summary\n\n'
    printf '| Check | Status |\n'
    printf '|---|---|\n'
...
step "shellcheck  deploy/tune-perf.sh" \
  "${DOCKER_RUN[@]}" shellcheck deploy/tune-perf.sh

section "JSON"
step "jsonlint  package.json" \
...
if [[ -f eslint.config.js || -f .eslintrc.json ]]; then
  step "eslint  server.js  public/app.js" \
    "${DOCKER_RUN[@]}" eslint --fix server.js public/app.js
else
...
step "semgrep  server.js + public/app.js" \
